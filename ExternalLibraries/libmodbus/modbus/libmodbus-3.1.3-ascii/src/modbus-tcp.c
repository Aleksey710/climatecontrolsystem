                                              ...
                                              const uint8_t *rsp, int rsp_length)
{
    ...
    if (req[0] != rsp[0] || req[1] != rsp[1]) {
        if (ctx->debug) {
            MODBUS_LOG("Invalid transaction ID received 0x%X (not 0x%X)\n",
...
//                    (rsp[0] << 8) + rsp[1], (req[0] << 8) + req[1]);
    ...
    if (rsp[2] != 0x0 && rsp[3] != 0x0) {
        if (ctx->debug) {
            MODBUS_LOG("Invalid protocol ID received 0x%X (not 0x0)\n",
                    (rsp[2] << 8) + rsp[3]);
...
static int _modbus_tcp_pi_connect(modbus_t *ctx)
{
    ...
    if (rc != 0) {
        if (ctx->debug) {
            MODBUS_LOG("Error returned by getaddrinfo: %s\n", gai_strerror(rc));
...
int modbus_tcp_accept(modbus_t *ctx, int *s)
{
    ...
    if (ctx->debug) {
        MODBUS_LOG("The client connection from %s is accepted\n",
...
int modbus_tcp_pi_accept(modbus_t *ctx, int *s)
{
    ...
    if (ctx->debug) {
        MODBUS_LOG("The client connection is accepted.\n");
...
static int _modbus_tcp_select(modbus_t *ctx, fd_set *rset, struct timeval *tv, int length_to_read)
{
    ...
    while ((s_rc = select(ctx->s+1, rset, NULL, NULL, tv)) == -1) {
        if (errno == EINTR) {
            if (ctx->debug) {
                MODBUS_LOG("A non blocked signal was caught\n");
...
modbus_t* modbus_new_tcp(const char *ip, int port)
{
    ...
    if (ip != NULL) {
        ...
        if (ret_size == 0) {
            MODBUS_LOG("The IP string is empty\n");
//            fprintf(stderr, "The IP string is empty\n");
        ...
        if (ret_size >= dest_size) {
            MODBUS_LOG("The IP string has been truncated\n");
...
modbus_t* modbus_new_tcp_pi(const char *node, const char *service)
{
    ...
    if (node == NULL) {
        ...
        if (ret_size == 0) {
            MODBUS_LOG("The node string is empty\n");
//            fprintf(stderr, "The node string is empty\n");
        ...
        if (ret_size >= dest_size) {
            MODBUS_LOG("The node string has been truncated\n");
//            fprintf(stderr, "The node string has been truncated\n");
    ...
    if (ret_size == 0) {
        MODBUS_LOG("The service string is empty\n");
    ...
    if (ret_size >= dest_size) {
        MODBUS_LOG("The service string has been truncated\n");
