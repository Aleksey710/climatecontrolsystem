...
//------------------------------------------------------------------------------------
//!
NativeModbusMasterHandler::NativeModbusMasterHandler(QObject *parent)
...
//------------------------------------------------------------------------------------
//!
NativeModbusMasterHandler::~NativeModbusMasterHandler()
{

}
//------------------------------------------------------------------------------------
//!
void NativeModbusMasterHandler::exequteRequest(ModbusRequest *request)
{
...
//------------------------------------------------------------------------------------
//!
bool NativeModbusMasterHandler::reconnect(const ModbusConnectionSettings &modbusConnectionSettings)
{
...
//------------------------------------------------------------------------------------
//!
void NativeModbusMasterHandler::requestExecutionErrorHandler()
{
    errorDataHandler();
...
//------------------------------------------------------------------------------------
//! Удалить предыдущий обработчик устройства
void NativeModbusMasterHandler::deleteModbusDevice()
{
    if (m_modbusDevice )
    {
...
//------------------------------------------------------------------------------------
//!
void NativeModbusMasterHandler::readRequest(const int serverAddress, QModbusDataUnit &readDataUnit)
{
    ...
    {
        if (!reply->isFinished())
            connect(reply, &QModbusReply::finished, this, &NativeModbusMasterHandler::readReady);
        else
            reply->deleteLater();
    } else {
...
//------------------------------------------------------------------------------------
//!
void NativeModbusMasterHandler::writeRequest(const int serverAddress, QModbusDataUnit &writeDataUnit)
{
    ...
    if ( auto *reply = m_modbusDevice->sendWriteRequest(writeDataUnit, serverAddress) )
    {
        if (!reply->isFinished())
        {
            connect(reply, &QModbusReply::finished, this, &NativeModbusMasterHandler::writeReady);
        } else {
            reply->deleteLater();
        }
    } else {
...
//------------------------------------------------------------------------------------
//!
void NativeModbusMasterHandler::readWriteRequest(const int serverAddress,
...
//------------------------------------------------------------------------------------
//!
void NativeModbusMasterHandler::readReady()
{
...
//------------------------------------------------------------------------------------
//!
void NativeModbusMasterHandler::writeReady()
{
    auto reply = qobject_cast<QModbusReply *>(sender());
...
//------------------------------------------------------------------------------------
//!
void NativeModbusMasterHandler::readReplyHandler(QModbusReply *reply)
{
    if (!reply)
    {
        //deleteModbusDevice();
...
//------------------------------------------------------------------------------------
//!
void NativeModbusMasterHandler::writeReplyHandler(QModbusReply *reply)
{
    if (!reply)
    {
        //deleteModbusDevice();
...
//!
/*
void NativeModbusMasterHandler::on_writeTable_currentIndexChanged(int index)
{
...
//------------------------------------------------------------------------------------
//!
QModbusDataUnit NativeModbusMasterHandler::readRequest() const
{
    const auto table =
...
//------------------------------------------------------------------------------------
//!
QModbusDataUnit NativeModbusMasterHandler::writeRequest() const
{
    const auto table =
        static_cast<QModbusDataUnit::RegisterType> (ui->writeTable->currentData().toInt());
